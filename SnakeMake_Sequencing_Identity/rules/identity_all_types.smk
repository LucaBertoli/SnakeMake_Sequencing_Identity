...
rule mpileup:
	input:
		...
		fasta=config["FASTA"]
	output:
		pileup=temp("{output_name}/{output_name}.all.types.pileup.gz"),
		new_target=temp("{output_name}/{output_name}.all.types.pileup.target.bed")
	shell:
		"""
...
rule identity_all_types:
	input:
		pileup="{output_name}/{output_name}.all.types.pileup.gz"
	output:
		out_report="{output_name}/IDENTITY_{output_name}_full_all.identity.all.types.pileup.report"
	params:
