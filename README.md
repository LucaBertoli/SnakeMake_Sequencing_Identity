...
## General description
Repository developed in the context of my PhD thesis.
...
- sequencer-assigned Qscore vs alignment-based Qscore
- Sequencer-assigned and alignment-based Qscores stratified by sequencing cycle
- combined sequencing cycle and assigned quality stratification of identity-based Qscores and fraction of emitted bases
- insert-size stratified sequencer-assigned Qscore and identity-based Qscore
- read-level alignment identity and error profiles 

## Key Concepts
Using the pysam library, the workflow extracts mismatches from the MD tag and indels from the CIGAR string. To discriminate between true genetic variantion or alignment artifacts from sequencing errors, mismatch and indels corresponding to user-specified variants are not considered errors. A BED file is used as input to restrict the analysis on user-defined regions (e.g. GIAB High Confidence Regions). This is performed to mitigate the effect of misalignments on misassembled or complex regions. 
