...
python $scripts_folder/IdentityRevelations.py $bam $vcf IDENTITY_${output_name}.tsv.gz
...
python $scripts_folder/Scatter_BAM.py $bam QUAL
...
python $scripts_folder/Identity_plot.py IDENTITY_${output_name}.tsv.gz $(dirname $bam)/$(basename $bam)_mean_quality.tsv.gz IDENTITY_$output_name
...
bash $scripts_folder/identity_calculalculation_all_types.sh $FASTA $bam $vcf $BED IDENTITY_${output_name}
