...
scripts_folder="/home/comparazione_kit/calcolo_identità_sequenze_Sequenziatori/scripts"
...
python $scripts_folder/Scatter_BAM_LogOfMeans.py $bam BAM
samtools index $(dirname $bam)/$(basename $bam .bam)_reads_30_up_LogOfMeans.bam
...
python $scripts_folder/IdentityRevelations.py $(dirname $bam)/$(basename $bam .bam)_reads_30_up_LogOfMeans.bam $vcf IDENTITY_${output_name}.tsv.gz
...
python $scripts_folder/Scatter_BAM_LogOfMeans.py $(dirname $bam)/$(basename $bam .bam)_reads_30_up_LogOfMeans.bam QUAL
...
python $scripts_folder/Identity_plot.py IDENTITY_${output_name}.tsv.gz $(dirname $bam)/$(basename $bam .bam)_reads_30_up_LogOfMeans.bam_mean_quality_LogOfMeans.tsv.gz IDENTITY_$output_name
...
bash $scripts_folder/identity_calculalculation_all_types.sh $FASTA $(dirname $bam)/$(basename $bam .bam)_reads_30_up_LogOfMeans.bam $vcf $BED IDENTITY_${output_name}
